...
class Admin{
  ...
  protected function PayPalSync(){
    ...
    foreach ($this->db->get("plans", ["free" => "0"]) as $data) {
      ...
      $merchantPreferences->setReturnUrl(Main::href("?success=true"))
          ->setCancelUrl(Main::href("?success=false"))
  ...
  protected function editor(){
    ...
    $this->footer();   
      ...
      protected function themeFiles(){
        ...
        foreach (new RecursiveDirectoryIterator(ROOT."/themes/{$this->config["theme"]}/") as $path){
          if(!$path->isDir() && $path->getFilename()!=="." && $path->getFilename()!==".." && (Main::extension($path->getFilename())==".php" || Main::extension($path->getFilename())==".css")){
            ...
            $current = !isset($_GET["file"]) || empty($_GET["file"]) ? "style" : $_GET["file"];
            if($path->getFilename()=="style.css") {
              $name = "Main Stylesheet";
              $data .= "<option value='$code' ".($current =="style" ? "selected":"").">$name ({$path->getFilename()})</option>";
      ...
      protected function currentFile(){
        $data=array();
        // Get File
        if(!empty($_GET["file"]) && $_GET["file"] != "style"){
